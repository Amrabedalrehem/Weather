...
class DataStorePermission(private val context: Context) : IDataStorePermission {
    ...
    override val wasPermissionRequested: Flow<Boolean> = context.permissionDataStore.data
        .map { it[PERMISSION_REQUESTED] ?: false }

    override val latitude: Flow<Double> = context.permissionDataStore.data
        .map { it[LATITUDE] ?: 0.0 }

    override val longitude: Flow<Double> = context.permissionDataStore.data
